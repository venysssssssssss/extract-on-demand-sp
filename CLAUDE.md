...
**IW51 flow:** `sap_iw51_dani.py` or `POST /api/v1/extractions/iw51` → `run_iw51_demandante()` copies the source workbook into the run directory, loads pending rows, distributes them across 3 fixed SAP sessions, runs in `process_parallel` mode by default for production stability, keeps `interleaved` as a safe fallback and `true_parallel` as an explicit experimental mode, appends per-row progress to `iw51_progress.csv`, reloads reruns with ledger `last-entry-wins`, classifies `failed_terminal` as terminal, reconciles workbook↔ledger at startup, synchronizes `FEITO=SIM` in batches on the working workbook copy, and persists `Iw51Manifest`. The workbook loader tolerates sparse blank rows in `Macro1` instead of treating the first empty line as EOF. If needed, recovery can rebuild `iw51_progress.csv` from `sap_session.log`.
...
- `iw59.py` — `Iw59ExportAdapter`: collects notes from CA/RL/WB CSVs filtered by `statusuar` + `allowed_status_values`, chunks them (default 5k for IGOR/MANU), runs IW59 extraction per chunk via clipboard multi-select (Win 11 default) with automatic file-upload fallback (Win 10), concatenates outputs into final per-object CSVs
- `iw51.py` — IW51 workbook-driven execution module for demandante `DANI`; reads `projeto_Dani2.xlsm`, creates a run-scoped working copy, applies the SAP creation flow with 3 fixed SAP sessions, defaults to `process_parallel` for production, supports `interleaved` as a safe fallback and `true_parallel` as an explicit experimental mode, writes resumable progress to `iw51_progress.csv`, syncs `FEITO=SIM` in batches on the working workbook, supports recovery from `sap_session.log`, and persists `Iw51Manifest`
- `dw.py` — DW complaints CSV flow; reads `ID Reclamação` from `BASE RECLAMAÇÕES 2026- ATUALIZADO(BASE) (1)(1).csv`, opens 3 SAP sessions, supports `parallel_mode` with fixed worker↔session affinity, scrapes tab `TAB02` text, writes `OBSERVAÇÃO` back to the same CSV, emits per-worker state in the manifest, and also writes `dw_observacoes_debug.csv` for raw observation inspection
...
- `service.py` — Factory functions: `create_session_provider(config)` selects provider based on `logon_pad.enabled`, `create_batch_orchestrator(output_root, config)` wires all dependencies, `run_batch_payload()`, `run_iw51_payload()`, `run_iw59_payload()`, `run_dw_payload()` for standalone execution
- `api.py` — FastAPI with IW69, IW51, IW59 and DW endpoints (run, manifest, curl for each flow). Also exposes job management (list, detail, cancel), schedules (list, upsert), and runners (list, detail) for the control plane
...
- **IW59 chunking:** Large note sets split into configurable chunks (default 5k for IGOR/MANU) to avoid SAP timeouts. Notes pasted via clipboard into SAP multi-select dialog on Win 11; automatic fallback to file upload (`btn[23]`) on Win 10 where clipboard paste truncates to a single note. Clipboard write is verified (retry 3x) and post-paste entry count is checked before proceeding
...
**Demandante terminology:** use `demandante` everywhere in contracts, config and API. Current supported demandantes are `IGOR`, `MANU`, `DANI`, `DW` and `KELLY`. `MANU` filters `IW59` input to `CA` notes whose `statusuar` is one of `ENCE`, `ENCE DEFE`, `ENCE DEFE INDE`, `ENCE DUPL`, `ENCE IMPR`, `ENCE INDE` or `ENCE PROC`; `IW69` must use the exact `from_date`/`to_date` supplied in the request.

**IW51 DANI flow:** uses fixed `RIWO00-QWRNUM=389496787`, reads `PN`, `INSTALAÇÃO` and `TIPOLOGIA` from `projeto_Dani2.xlsm`, runs with `session_count=3` and `execution_mode=process_parallel` by default, preserves the original workbook by operating on a run-scoped copy, records resumable status in `iw51_progress.csv`, treats `success`, `rejected` and `failed_terminal` as terminal on rerun, syncs `FEITO=SIM` in batches of `25` by default (config overrides to `100`) on the working copy, supports `worker_start_stagger_seconds`, `worker_heartbeat_seconds`, `worker_item_timeout_seconds`, `worker_restart_limit`, `session_rebuild_backoff_seconds` and `message_filter_retry_window_seconds`, keeps `interleaved` disponível como fallback seguro e `true_parallel` como modo explícito/experimental, suporta recuperação do ledger a partir do `sap_session.log`, e não interrompe a leitura da `Macro1` por causa de linhas vazias intermediárias. `inter_item_sleep_seconds` é configurável (code default `0`, config sets `1`).

**IW59 KELLY flow:** `POST /api/v1/extractions/iw59` with `demandante=KELLY` → `execute_modified_by_brs()` reads BRS values from `brs_filtrados.csv`, computes modified date windows via `compute_kelly_modified_date_ranges()` with `transition_business_day=5`, chunks BRS into groups of 100, multi-selects on `AENAM` field instead of `QMNUM`, runs per-window per-chunk extraction, concatenates all outputs into a single `iw59_kelly_*.csv`.
