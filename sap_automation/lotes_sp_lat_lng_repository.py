    ...
    def save_rows(self, rows: list[dict[str, str]]) -> int:
        ...
        with self.engine.begin() as conn:
            if normalized_rows:
                ...
                conn.execute(delete(instalacoes_coordenadas_sp).where(instalacoes_coordenadas_sp.c.instalacao.in_(keys)))
                conn.execute(insert(instalacoes_coordenadas_sp), normalized_rows)
