...
- `VALIDA_DANI`: usa `sap_valida_dani.py` ou `/api/v1/extractions/valida-dani` para ler a planilha original da Dani em `output/runs/20260330T171500/iw51/working/projeto_Dani2.xlsm` na máquina que executa a aplicação; os valores da coluna `A` são limpos removendo vírgulas, deduplicados em ordem, divididos em lotes de no máximo `5000` e colados no campo `Cliente/KUNUM` da `IW59` com `Criado por = BR0041761455`. O fluxo consolida os TXT retornados, preserva a coluna `NOTA`, gera `resultado_validacao.csv` com `NOTAS_SAP` e grava `valida_dani.log` + `valida_dani_chunks.csv` no diretório de saída.
...
- com `fetch_installations_only=false`, reaproveita `output/runs/{run_id}/medidor/input/MEDIDOR_INSTALLATIONS.csv` quando o arquivo já existir para o mesmo `run_id`; se não existir, consulta o banco
- `MEDIDOR` e `MEDIDOR_SP` sempre usam SAP Logon pad na conexão RP1 `H181 RP1 ENEL SP CCS Produção (without SSO)`
