...
- `VALIDA_DANI`: usa `sap_valida_dani.py` para ler a planilha original da Dani nas colunas `A/B/C` (`CLIENTE`, `INSTALAÇÃO`, `DESCRIÇÃO`), consultar a `IW59` em lotes de `5000` clientes no campo `Cliente/KUNUM` com `Criado por = BR0041761455`, consolidar os TXT retornados e gerar `resultado_validacao.csv`; a validacao exige que cada linha da Dani exista no retorno SAP com o mesmo trio `CLIENTE + INSTALACAO + DESCRICAO` (normalizando zeros à esquerda e acentos). O run também grava `valida_dani.log` e `valida_dani_chunks.csv` no diretório de saída.
